...
namespace RestFul
{
    ...
    public abstract class Mssql
    {
        ...
        public Mssql()//可以用来初始化connectionString
        {
            //connectionString = ConfigurationManager.ConnectionStrings["MssqlConnectionString"].ConnectionString;
        }
        ...
        public static int ExecuteInsertSql(string SQLString)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand(SQLString, connection))
